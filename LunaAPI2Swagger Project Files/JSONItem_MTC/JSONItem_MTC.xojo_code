...
Protected Class JSONItem_MTC
	...
	#tag Method, Flags = &h0
		Sub Remove(name As String)
		  ...
		  dim key as string = NameToKey( name )
		  'ObjectValues.Remove( key ) //changed the order, which is difficult if you want to compare with the output of the node.js api2swagger executable
		  Dim dicTMP As New Dictionary
		  For i As Integer=0 to ObjectValues.Count-1
		    if ObjectValues.Key(i)<>key Then
		      dicTMP.Value(ObjectValues.Key(i))=ObjectValues.Value(ObjectValues.Key(i))
		    end if
		  Next
		  ObjectValues=dicTMP
	...
	#tag Method, Flags = &h21
		Private Sub Validate(value As Variant)
		  ...
		  case Variant.TypeInteger, Variant.TypeInt64
	...
	#tag Constant, Name = Version, Type = Double, Dynamic = False, Default = \"2.9", Scope = Public
	#tag EndConstant
